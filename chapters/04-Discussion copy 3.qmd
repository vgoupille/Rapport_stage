...
Liste des idées a mettre dans la discussion :

- rappel de l'obejctif : valider Microsplit , DOL (nous pas eu le temps encore mais sera fait prochainement)

   - sequençage ok mais dupli, non equilibré
   - pourariat ameliorer controle qualité (contaminant ...) 
   - trimming ok marche bien permit eliminer reads de mauvaise 
   - STARsolo ok , envisageable de changer quelques parametres et voir qu'elle effet cela a sur les reads 
   - bonne proportion mRNA 
   - filtrges montre que globalement les cellules en milieu stressé ont moins de reads et plus sensible au filtrage ce qui a justifié l'utilisation d'un seuil par condition , a noté que cela signifie que l'on a supprimé des avec plus haut UMIS dans conditions riche de fer par rapport ceux fible en fer 
   - certaines variabilité entre replica techique qui creait une variance importante parfois au sein d'un replica biologique mais apres le filtrga seulement le top donc enleve les bas performants
   - on observait que ce nombre de UMIs et de gene detectés varie entre certain replica biologique  mais surtout en focntion du stade croissance de la poplation, on peut lié cela aux conditions peut etre  que en condtion limitante de ressources cela reduit la capacité de la population a etre active et va plutot plutot mettre en place des strategie de survie et de protection  comme le suggere ... 
ceci est d'autant plus vrai que l'on retrouve  ... comme genes qui parraisse differentiellement expreimé  dans les conditions stress feriruq tardibe ; alors que temps 1 dans mileiu appauvri pas et inversement dans 
   - 
   - abouti a 160 cellules par condition soit un total de 3000 cellules ici deja on peut s'interogé si cela est suffisant pour faire des analyses statistiques Avons-nous capturé suffisamment de cellules par condition (~160) pour détecter une DOL ? L’absence de clustering sera t'il le reflet d’une régulation “bruyante” (noisy) ou simplement d’un échantillonnage trop faible ? 
 
   - 



-avantage : peu de cellules contrairement au etudes eucaryotes pour lesquelles on a centaindes de millier de cellules voir millions . 

ici geneome procaryotes,  3000 cellules, avatages peut totalement tourné en local et peu de gène donc peut etre utile d'un point de vue pedagogique 

- point de vue logiciel j'ai testé R ou python , ici presenté python avec scanpy uniquement m'a permis de prendre en main ce language que je maitrissait moins . 
-j'ai egalement appris à creer des container docker, devcontainer permet d'assurer la reproductibilité des resultats, facilite pour une perpective de valorisation scientifique 

toujours dans cette perpectives, j'ai appris a utuiliser quarto pour relaiser ce rapport et (un template sera disponible pour reproduire facilement un rapport, et facilite l'accesibilté via html) 


- difficultés : 
- j'ai eu les resulats assez tardivement, donc je voulais mentrainer sur les resulats d'autres articles mais peu fournissent les metadata associé a leur données brutes donc impossibiltés (pourtant paaru dans des grands journaux )


  - en plus de pas fournir le code pour l'analyse de leur données, les articles partages pas leurs tailles de sous libraires qu'il vissait donc ne sait pas si ils filtres bien pour s'approché da la tailles de "cellule réelles estimé" => filtre t'il a un seuil ou ils n'ont pas forcement des cellules uniques 
 doit on faire confiance dans leur estimation ? 

  - a savoir que cela reste tres subjectif la methodes de filtrages : comment distinguer cellules reelles de prenont un exemple parfois retrouve que certaines cellules de la conditions non stressé a des cellules quis eraient qualifié de mauvaise quzlité ( sur le critere du nombre UMIs)
  - cela soulève de nombreuse questions : s'aggisait t'il de cellules reelles ou de cellules qui ont ete mal qualifiées ? 
  - a cela s'ajoute la question si cellules dormantes captes pas/ peu  de signal, 
  il peut sagir pourtant d'une division certaine du travail en se protegeant  (la non activité est une activité en tant que telle pour maximiser la survie, la fitness de la population )
  -existe 
  - est ce que l'estimation du nombre de cellules via aggrega cellulaire est correcte ou bien sous estime le nombre de cellules reellement barcodes ? mais si il s'agit effeccitvement d'aggrga cellulaire contenant plsueirus cellules le barcode peut il etre considéré comme uniques ? 

nous avons testé avec 3000 cellules pour maximiser le nombre de cellules pour valider la capacité de sequençage pour leqeul c'etais la premiere que l'on testait cette methode. 

methode de depletition de ARNr permettrait de recuperer plus de signal mRNA avant le sequençage 
de plus, long read technologie permettrait d'observer opéron est un groupe de gènes transcrits en une seule molécule d’ARNm polycistronique, permettrait de voir voir gene coregulés ( apres inspection visuel de la matrice genes situé a proxité direct semblaient avoir un mobre de count surexprrime par rapport a leur voisins , il serait interrsant d"analyser cela plus en détail). De meme ici il serait interrsant de ce focalité sur des genes regulation , integrase ou petit tra,script regulateur. 

-ici niveau ARN mais couplé a methodes de single cell, proteomic , metabolomic perpective lointaine s


Microsplit est une methode inovante avec de nombreux avantage comme sa faiclé d'utilisation, pas besoin de beaucoup de materiel, et peu le faire sur de nombreuse cellules 10 0000, travaille encourageant pour le futur mais qu'il faut garder en tete toutes ses limites pour ne pas se tromper dans l'interpretation des resultats. 

-si cellules mortes ? l'argage de molecules une forme de division du travaille ? 

- il serait interessant de tester avec un filtrgae moins stringent voir si on observe la meme hhsoe ou pas ? 
-avons nous assez de cellules par condition 160 unqiement ( si on obseve pas de clustering est ce que cela sera une regulation que l'on poura qualifié noisy regulation ou bien la puissance de caractérisé car tirage alétoire de cellules est relativement faible sur toute la population cellulaire est ce que sera capable de detecter une potentielle dol si sous population avec seulement quelle cellules qui non pas ete tirées ? 

ici milieu de culture, donc est ce que suffisament de gradient pour voir cluster aparaitre comme pourrait en condition plus variable, naturel ?

est ce que assez de capture de mRNA pour pouvoir clusterisé ta t de question 

Beaucoup de questions qui restent ouvertes 

- cela suggere besoin de tester  ave autres methodes de single cell pour confirmer une obserbatopn 
 le methodes d'isolement parraissent probablement plus adapté , evolation rapide des methodes 
  
  - variabilité entre les replica technique sont je trouve assez importantes, si on filtre par replica techniuque et non par par replicas biologique cela garderai les top cellules des replica stzhnique mais pourrait au parfois des distribtion binomaile si certains replica technique ont ete sous performant dans replica biolo et cela rendrait l'analyse et l'interpreation encore plus compliqué 



perspective amelioration :
- si refait l'experimentation il serait interresant de faire des bulks et des blancs pour améliorer les stats comme dans cyriaque
- possibilité de check contamination si potentiel autres ARN contaminants avec diffrentes outils 
- tester le preprint pour l'écture des barcodes optimisées via cigar motif via BARQC , celui ci permet egalement de check la varibilité dans chaque puit , j'ai fait une shiny app inspiré de celui non presenté ici qui me permet de visualisé les UMIs dans chaque puits et appliqués filtres pour voir si 
- tester completement le pipeline bacSC, j'ai du modifier une partie de leur code car packages numpy n'etait plus adapté pour gerer les matrices sparces. 
- deja faire analyse de la DOL en etudiant chaque sample  de maniere independante 

- test d'autres outils : SCVI , ou scanorama pour modelisation, intgration regrssion des données , voir si beosin de par nombre de count 
- puis ensuite faire analyse Gene Ontology , trajectoire cellulaire (citer article), metabolomic patway... pseudobulk analysis ...
- pipeline nfcore pour les etapes de trimming et qualité 
...
- autre info diverse 
  - taux de duplication des barcodes qui est differentes entre les 4 indices (besoin de comprendre pourquoi )
  - taux de saturation discussion , serai interessant de voir si on a pas des difference entre les conditions ,
  -  voir le nombre UMIs que je recupere par condition 
...
## Biological Interpretation of Transcriptional Responses

### Principal Component Analysis and Temporal Dynamics
...
## Technical Challenges and Implications for Division of Labor Detection
