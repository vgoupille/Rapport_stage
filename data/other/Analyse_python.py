...
# Récupérer les valeurs uniques de ODt
odt_values = adata.obs["ODt"].unique()

for odt in odt_values:
    # Filtrer pour ce ODt
    df_odt = adata.obs[adata.obs["ODt"] == odt].copy()

    # Regrouper les données par RepBio et RepTech_suffix, compter les occurrences
    counts = (
        df_odt.groupby(["RepBio", "RepTech_suffix"]).size().reset_index(name="counts")
    )

    # Pivot pour avoir RepTech en colonnes, RepBio en index
    ...
    ax.set_title(f"Counts by RepBio and RepTech (ODt={odt})", fontsize=14)
    ax.set_xlabel("RepBio", fontsize=12)
    ax.set_ylabel("Counts", fontsize=12)
    ax.legend(title="RepTech", bbox_to_anchor=(1.05, 1), loc="upper left")
    plt.tight_layout()
    plt.show()
...
# Créer la colonne suffixe RepTech si pas déjà faite
adata.obs["RepTech_suffix"] = adata.obs["RepTech"].str.split("_").str[-1]

# On compte les occurrences par RepBio, RepTech_suffix et ODt
counts = (
...
g.set_axis_labels("RepBio", "Counts")
g.set_titles("ODt = {col_name}")
g._legend.set_title("RepTech suffix")
plt.tight_layout()
plt.show()
...
# Créer la colonne suffixe si besoin
adata.obs["RepTech_suffix"] = adata.obs["RepTech"].str.split("_").str[-1]

# Calcul des counts cumulés par ODt, RepBio et RepTech_suffix
counts = (
    adata.obs.groupby(["ODt", "RepBio", "RepTech_suffix"])
    .size()
    .reset_index(name="counts")
...
# Récupérer les valeurs uniques d'ODt
...
for odt in odt_values:
    ...
    ax = pivot_df.plot(kind="bar", stacked=True, figsize=(10, 6), colormap="tab20")
    ax.set_title(
        f"Counts cumulated by RepBio and RepTech_suffix (ODt={odt})", fontsize=14
    )
    ax.set_xlabel("RepBio", fontsize=12)
    ax.set_ylabel("Counts", fontsize=12)
    ax.legend(title="RepTech suffix", bbox_to_anchor=(1.05, 1), loc="upper left")
    plt.tight_layout()
    plt.show()
...
# Créer la figure avec n sous-graphes côte à côte
fig, axes = plt.subplots(1, n, figsize=(6 * n, 6), sharey=True)

# Pour gérer le cas où n=1 et axes n'est pas un tableau
if n == 1:
    axes = [axes]

# Récupérer toutes les catégories de RepTech_suffix pour légende et couleurs cohérentes
all_suffixes = counts["RepTech_suffix"].unique()

# Créer une palette unique pour tous les graphes (même ordre)
colors = plt.get_cmap("tab20").colors
color_map = {
    suffix: colors[i % len(colors)] for i, suffix in enumerate(sorted(all_suffixes))
}
...
for ax, odt in zip(axes, odt_values):
    ...
    # Réordonner colonnes pour la même palette
    pivot_df = pivot_df.reindex(columns=sorted(all_suffixes), fill_value=0)

    # Plot empilé avec couleurs fixes
    pivot_df.plot(
        kind="bar",
        stacked=True,
        ax=ax,
    ...
    ax.set_title(f"ODt = {odt}", fontsize=14)
    ax.set_xlabel("RepBio", fontsize=12)
    ax.set_ylabel("Counts", fontsize=12)
    ax.set_xticklabels(
        ax.get_xticklabels(), rotation=0, ha="center"
    )  # étiquettes horizontales

# Ajuster l'espace entre les subplots
plt.tight_layout(rect=[0, 0, 0.85, 1])

# Légende commune sur la droite
...
for i, odt in enumerate(odts):
    ...
    for j, repbio in enumerate(repbios):
        ...
        violin_color = "gray"  # gris clair constant
...
if "RepTech_suffix" not in adata.obs.columns:
    adata.obs["RepTech_suffix"] = adata.obs["RepTech"].str.split("_").str[-1]
...
# Si un seul axe, le mettre dans une liste
if n == 1:
    axes = [axes]

total_all = 0  # Pour le total global
...
for ax, odt in zip(axes, odt_values):
    ...
    # Ajouter (n=...) au-dessus des barres
    bar_sums = pivot_df.sum(axis=1)
    for i, val in enumerate(bar_sums):
        ax.text(i, val + 1, f"(n={int(val)})", ha="center", va="bottom", fontsize=6)

    # Annotation du total par plot
    total_plot = bar_sums.sum()
    total_all += total_plot
    ax.text(
        0.5,
        0.98,
        f"Total barcodes: n={int(total_plot)}",
...
fig.legend(
    handles,
    labels,
    title=f"RepTech_suffix\nTotal across all ODt: n={int(total_all)}",
    loc="center right",
    fontsize=9,
    title_fontsize=10,
)
...
# Palette tab20
all_suffixes = sorted(df["RepTech_suffix"].unique())
colors_tab20 = plt.get_cmap("tab20").colors
color_map = {
    suffix: colors_tab20[i % len(colors_tab20)] for i, suffix in enumerate(all_suffixes)
}
palette = [
    color_map[suffix] for suffix in all_suffixes
]  # palette dans l'ordre des suffixes
...
# Palette tab20 personnalisée
reptech_suffixes = sorted(df["RepTech_suffix"].unique())
...
### filter to conserve only genes expressed in 3 cells or more


### filter to conserve only
...
# # Create mask for cells to keep (n_counts >= 100)
# cells_to_keep = adata.obs["n_counts"] >= 200
