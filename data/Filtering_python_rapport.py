...
# Preprocessing
df = adata_fcell_100UMI.obs.copy()
df["n_counts"] = adata_fcell_100UMI.obs["n_counts"]
...
# Total par CultureMedium
total_by_medium = (
    adata.obs.groupby("CultureMedium")
    .size()
    .reindex(["M9", "M9F"])  # pour garder l'ordre
    .fillna(0)
    .astype(int)
...
adata_fcell_100UMI_noOut_Reptech = adata_filtered.copy()
...
adata_fcell_Reptech = adata_filtered.copy()


# %% see min max n counts by group
adata_fcell_Reptech.obs["n_counts"].describe()

# %%
adata_fcell_Reptech.obs.groupby("RepTech")["n_counts"].describe()


# %%  see min max n counts by group OD
adata_fcell_Reptech.obs.groupby("OD")["n_counts"].describe()

# %%
sc.pl.violin(
    adata_fcell_Reptech, ["n_counts"], groupby="RepTech_suffix", multi_panel=True
)


# %% remove outliers

adata_fcell_RepTech_noOut = adata_fcell_Reptech.copy()


# %%
test = adata_fcell_RepTech_noOut.obs["RepTech"].value_counts()
pd.DataFrame(test)
...
print(
    f"📊 Number of cells before removing outliers: {adata_fcell_RepTech_noOut.n_obs:,} cells"
)
...
for od, n_cells in od1_groups.items():
    highest = adata_fcell_RepTech_noOut.obs[
        adata_fcell_RepTech_noOut.obs["RepTech"] == od
...
cells_to_keep = ~adata_fcell_RepTech_noOut.obs.index.isin(cells_to_remove)

# Apply filtering
adata_fcell_RepTech_noOut = adata_fcell_RepTech_noOut[cells_to_keep].copy()

print(f"📊 After removing outliers: {adata_fcell_RepTech_noOut.n_obs:,} cells")
print(f"🗑️ Removed {len(cells_to_remove)} outlier cells")

# Show violin plot after outlier removal
sc.pl.violin(
    adata_fcell_RepTech_noOut, ["n_counts"], groupby="RepTech", multi_panel=True
)
...
# %%
adata_fcell_RepTech_noOut


# %%
# nombre de cellules total
n_cells_before_filtering = adata_fcell_Reptech.n_obs
# %%
n_cells_after_filtering = adata_fcell_RepTech_noOut.n_obs
...
# Preprocessing
df = adata_fcell_RepTech_noOut.obs.copy()
df["n_counts"] = adata_fcell_RepTech_noOut.obs["n_counts"]
df["RepTech_suffix"] = df["RepTech"].astype(str).str.split("_").str[-1]
df["ODt"] = df["ODt"].astype(str)
df["RepBio"] = df["RepBio"].astype(str)
df["RepTech_suffix"] = df["RepTech_suffix"].astype(str)
...
# Grouper les données
counts = (
    adata_fcell_RepTech_noOut.obs.groupby(["ODt", "RepBio", "RepTech_suffix"])
    .size()
    .reset_index(name="counts")
)
...
# Total par CultureMedium
total_by_medium = (
    adata_fcell_RepTech_noOut.obs.groupby("CultureMedium")
    .size()
    .reindex(["M9", "M9F"])  # pour garder l'ordre
    .fillna(0)
    .astype(int)
...
adata_fcell_Reptech.var["Gene Type"].value_counts()

adata_fcell_Reptech.var["Gene Type"].unique()


# %%
adata1 = adata_fcell_RepTech_noOut.copy()
